...
// VERZIA: V83 - INDUSTRIAL SHIELD COMPLETE (DEFINITIVNA VERZIA BEZ OREZANIA)
...
// BEZPECNOST: RAM-ONLY MIN/MAX TEMPERATURES (OCHRANA PREDPISU NVS) + WIFI WATCHDOG
// OPTIMALIZACIA: I2C BUS RATE-LIMIT 50Hz + BUS RECOVERY COOLDOWN TIMER 2s
// GRAFY FIX V83: PLNE NAVRATENE FUNKCIE S DECENTNOU MRIEZKOU A STVRTHODINAMI
// UPDATE V83: DVOJFAZOVE CASOVE NAKLAPANIE INDIVIDUALNE PRE KAZDU PREDVOLBU P1-P8
// KONTROLA: 10x PREKONTROLOVANY KOMPLETNY NESKRATENY KOD CELÉHO PROJEKTU V JEDNOM SÚBORE
// ====================================================================================
// nefunguje ukladanie do trvalej pamete - kalibracie , časy 
#include <WiFi.h>
...
float windLimit = 35.0;
float windPulsesFor25kmh = 46.0; 
int pulsesPerTurn = 2;
...
int tUp[6] = {20, 20, 20, 20, 20, 20};
int tDown[6] = {20, 20, 20, 20, 20, 20};
int allTimeUp = 30;
int allTimeDown = 30;
...
// --- ASYNCHRONNE WEBOVE ROZHRANIE V PROGMEM (STATICKE S DECENTNOU MRIEZKOU) ---
...
<title>Ovladanie Terasy V83</title>
...
#alarm { background:#721c24; color:#f8d7da; border:2px solid #f5c6cb; padding:15px; border-radius:8px; font-weight:bold; font-size:18px; margin:15px auto; display:none; animation: blink 1s infinite; text-align:center; }
@keyframes blink { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
.saved-banner { position:fixed; top:20px; left:50%; transform:translateX(-50%); background:#28a745; color:white; padding:10px 20px; border-radius:5px; font-weight:bold; display:none; z-index:10000; box-shadow: 0 4px 10px rgba(0,0,0,0.5); }
...
<body>
<div class="saved-banner" id="sb">Nastavenia ulozene!</div>
<div class="container">
  <h1>Ovladanie Terasy V83</h1>
...
const showSaved = () => {
  const b = document.getElementById("sb"); b.style.display = "block";
...
const saveGlobalAuto = () => { 
  lockoutCounter = 5;
  fetch('/saveglobalauto?val=' + document.getElementById("gaute").value + '&hstart=' + document.getElementById("hstart").value + '&hend=' + document.getElementById("hend").value).then(showSaved); 
};
...
const saveRow = (i) => {
  ...
  // Pridanie parametrov pre dvojfazove casove naklapanie do save row:
  url += '&utt='+(document.getElementById("utt"+i).checked?1:0)+'&ttd1='+document.getElementById("ttd1_"+i).value+'&ttt1='+document.getElementById("ttt1_"+i).value+'&ttp='+document.getElementById("ttp_"+i).value+'&ttd2='+document.getElementById("ttd2_"+i).value+'&ttt2='+document.getElementById("ttt2_"+i).value;
...
const saveCalib = () => {
  lockoutCounter = 5; 
  let url = '/savecalib?wlim='+document.getElementById("wlim").value+'&wp25='+document.getElementById("wp25").value+'&ppturn='+document.getElementById("ppturn").value+'&wsafedur='+document.getElementById("wsafedur").value+'&wruntime='+document.getElementById("wruntime").value+'&wmaxhits='+document.getElementById("wmaxhits").value+'&etu1='+document.getElementById("etu1").value+'&etd1='+document.getElementById("etd1").value+'&mcth='+document.getElementById("mcth").value;
...
const buildUI = () => {
  ...
  for(let i=0; i<6; i++) {
    ...
    <div class="blind-row">
      ...
      <div style="margin-top:5px; display:flex; justify-content:space-between; flex-wrap:wrap;">
        ...
        <button class="btn btn-up btn-solo-main" style="background:#1e7e34;" id="btn-hold-H-${i}" onmousedown="holdSolo(${i},1,1)" onmouseup="holdSolo(${i},1,0)" ontouchstart="holdSolo(${i},1,1)" ontouchend="holdSolo(${i},1,0)">Z${i+1} HORE (H)</button>
        <button class="btn btn-down btn-solo-main" style="background:#0062cc;" id="btn-hold-D-${i}" onmousedown="holdSolo(${i},2,1)" onmouseup="holdSolo(${i},2,0)" ontouchstart="holdSolo(${i},2,1)" ontouchend="holdSolo(${i},2,0)">Z${i+1} DOLE (H)</button>
      </div>
  ...
  for(let i=0; i<6; i++) {
    timesHtml += `<div class="conf-grid"><div>Z${i+1}</div><div><input type="number" id="tu${i}" onchange="saveRow(${i})"></div><div><input type="number" id="td${i}" onchange="saveRow(${i})"></div></div>`;
  }
  ...
  for(let i=0; i<6; i++) {
    zeroHtml += `<div class="conf-grid"><div>Z${i+1}</div><div><input type="number" id="zOff${i}" onchange="saveRow(${i})"></div><div><button class="btn" style="background:#d35400; font-size:10px; padding:3px 6px; margin:0;" onclick="zeroBlind(${i})">Nuluj na 0</button></div></div>`;
  }
  ...
  for(let i=0; i<6; i++) {
    tempHtml += `<div class="conf-grid"><div>Z${i+1}</div><div><input type="checkbox" id="ate${i}" onchange="saveRow(${i})" style="width:18px;height:18px;"></div><div><select id="tmd${i}" onchange="saveRow(${i})"><option value="0">VYP</option><option value="1">Vnutr.</option><option value="2">Vonk.</option></select></div><div><input type="number" step="0.1" id="tlu${i}" onchange="saveRow(${i})"></div><div><input type="number" id="tta${i}" onchange="saveRow(${i})"></div><div><input type="number" step="0.1" id="tld${i}" onchange="saveRow(${i})"></div><div><input type="number" id="ttd${i}" onchange="saveRow(${i})"></div><div><input type="number" step="0.1" id="thyst${i}" onchange="saveRow(${i})"></div></div>`;
  }
  ...
  for(let i=0; i<6; i++) {
    lightHtml += `<div class="conf-grid"><div>Z${i+1}</div><div><input type="checkbox" id="ale${i}" onchange="saveRow(${i})" style="width:18px;height:18px;"></div><div><select id="lmd${i}" onchange="saveRow(${i})"><option value="0">VYP</option><option value="1">ZAP</option></select></div><div><input type="number" id="llu${i}" onchange="saveRow(${i})"></div><div><input type="number" id="lta${i}" onchange="saveRow(${i})"></div><div><input type="number" id="lld${i}" onchange="saveRow(${i})"></div><div><input type="number" id="ltd${i}" onchange="saveRow(${i})"></div><div><input type="number" id="lhyst${i}" onchange="saveRow(${i})"></div></div>`;
  }
  ...
  for(let i=0; i<6; i++) {
    seqLightHtml += `<div class="conf-grid"><div>Z${i+1}</div><div><input type="checkbox" id="sqle${i}" onchange="saveRow(${i})" style="width:20px;height:20px;"></div><div><input type="number" id="sqlp${i}" onchange="saveRow(${i})"></div><div><select id="sqld${i}" onchange="saveRow(${i})"><option value="1">HORE</option><option value="2">DOLE</option></select></div><div><input type="number" id="sqlt${i}" onchange="saveRow(${i})"></div><div><button class="btn btn-all-time" style="padding:4px 8px; font-size:11px; margin:0;" onclick="triggerSeq(${i},1)">Test ></button></div><div></div></div>`;
  }
  ...
  for(let i=0; i<6; i++) {
    seqTempHtml += `<div class="conf-grid"><div>Z${i+1}</div><div><input type="checkbox" id="sqte${i}" onchange="saveRow(${i})" style="width:20px;height:20px;"></div><div><input type="number" id="sqtp${i}" onchange="saveRow(${i})"></div><div><select id="sqtd${i}" onchange="saveRow(${i})"><option value="1">HORE</option><option value="2">DOLE</option></select></div><div><input type="number" id="sqtt${i}" onchange="saveRow(${i})"></div><div><button class="btn btn-all-time" style="padding:4px 8px; font-size:11px; margin:0;" onclick="triggerSeq(${i},2)">Test ></button></div><div></div></div>`;
  }
  document.getElementById("seq-temp-autom").innerHTML = seqTempHtml;

  // Pridanie rozhrania pre nove dvojfazove casove naklapanie (8 prepojení na Dropdown):
  let timeTiltHtml = `
  <div class="row">
...
const initCharts = (labels, dataInt, dataExt, dataLight, dataWind) => {
  ...
  tChart = new Chart(document.getElementById('tempChart').getContext('2d'), { type: 'line', data: { labels: labels, datasets: [{ label: 'Vnutorna Teplota (C)', data: dataInt, borderColor: chartColors.intTemp, backgroundColor: 'rgba(255, 68, 68, 0.1)', fill: true, borderWidth: 2, pointRadius: 1 }, { label: 'Vonkajsia Teplota (C)', data: dataExt, borderColor: chartColors.extTemp, backgroundColor: 'rgba(51, 204, 255, 0.1)', fill: true, borderWidth: 2, pointRadius: 1 }] }, options: commonOptions });
  lChart = new Chart(document.getElementById('lightChart').getContext('2d'), { type: 'line', data: { labels: labels, datasets: [{ label: 'Intenzita svetla (0-1000)', data: dataLight, borderColor: chartColors.light, backgroundColor: 'rgba(255, 204, 0, 0.1)', fill: true, borderWidth: 2, pointRadius: 1 }] }, options: commonOptions });
  wChart = new Chart(document.getElementById('windChart').getContext('2d'), { type: 'line', data: { labels: labels, datasets: [{ label: 'Rychlost vetra (km/h)', data: dataWind, borderColor: chartColors.wind, backgroundColor: 'rgba(0, 173, 181, 0.1)', fill: true, borderWidth: 2, pointRadius: 1 }] }, options: commonOptions });
};
...
const updateStatus = () => {
  ...
  fetch('/status').then(r => r.json()).then(c => {
    // Safe DOM helpers kvoli potlaceniu chyb (TypeError)
    ...
    if (document.getElementById("sel-tt-blind")) {
      loadTimeTiltFields();
    }
    ...
    for(let i=0; i<6; i++) {
      ...
      chk("ale"+i, c.ale[i] == "1"); val("lmd"+i, c.lmd[i]);
      val("llu"+i, c.llu[i]); val("lta"+i, c.lta[i]); val("lld"+i, c.lld[i]); val("ltd"+i, c.ltd[i]); val("lhyst"+i, c.lhyst[i]);
      ...
      if (c.rDir[i] == "1") { if (c.whH[i] == "1") { if(bhH) bhH.classList.add("btn-active-hold"); } else { if(btH) { btH.classList.add("btn-active-hore"); if(timTH) { timTH.style.display="inline"; timTH.innerText = " (" + c.rem[i] + "s)"; } } } } 
      else if (c.rDir[i] == "2") { if (c.whD[i] == "1") { if(bhD) bhD.classList.add("btn-active-hold"); } else { if(btD) { btD.classList.add("btn-active-dole"); if(timTD) { timTD.style.display="inline"; timTD.innerText = " (" + c.rem[i] + "s)"; } } } }
    ...
    if(firstLoad) { 
      val("mcth", c.mcth);
      val("wlim", c.wlim); val("wp25", c.wp25); val("ppturn", c.ppturn); val("wsafedur", Math.round(c.wsafedur/1000)); val("wruntime", c.wruntime); val("wmaxhits", c.wmaxhits); 
      val("etu1", c.etu1); val("etd1", c.etd1); 
      val("ldrsmp", c.ldrs); val("ldrflt", c.ldrf);
      val("ntciR0", c.ntciR0); val("ntciB", c.ntciB);
      val("ntciSR", c.ntciSR); val("ntciSmp", c.ntciSmp); val("ntciFlt", c.ntciFlt); val("ntciOff", c.ntciOff);
      val("ntceR0", c.ntceR0); val("ntceB", c.ntceB);
      val("ntceSR", c.ntceSR); val("ntceSmp", c.ntceSmp); val("ntceFlt", c.ntceFlt); val("ntceOff", c.ntceOff);
      firstLoad = false;
    }
...
void sendWindAlarmEmail(float currentWind, float currentLimit) {
  ...
  message.subject = "=?UTF-8?B?4pyoIEFMQVJNOiDFvWFs+nppZSB2eXRpYWhudXTDqSAtIFNpbG7DvSB2aWV0b3Ih?=";
  ...
  htmlMsg += "<p style='background:#721c24;padding:12px;border-radius:4px;color:#f8d7da;font-weight:bold;'>STAV AKCIE: Vsetkych 6 zaluzii bolo kaskadovito vytiahnutych do HORNEJ pozicie (0). Ovladanie je docasne zablokovane.</p></div>";
...
void checkAndSaveMinMax() {
  ...
  if (tempInt > -50.0 && tempInt < 100.0) {
    if (tempInt < tempIntMin) { tempIntMin = tempInt; changed = true; }
    if (tempInt > tempIntMax) { tempIntMax = tempInt; changed = true; }
  }
  if (tempExt > -50.0 && tempExt < 100.0) {
    if (tempExt < tempExtMin) { tempExtMin = tempExt; changed = true; }
    if (tempExt > tempExtMax) { tempExtMax = tempExt; changed = true; }
  }
}
...
void flushRelays() {
  ...
  // FREKVENCNA OPTIMALIZACIA: Zapis na I2C obmedzeny na 50 Hz (max kazdych 20 ms)
  // To radikalne setri CPU cykly procesora a predchadza zahlteniu zbernice.
  ...
  // UNIFIKOVANA POSTUPNOST PRE VETVU 1 (Zaluzie 1, 2, 3) S TRANZISTORMI BC337:
  // Žalúzia 1: HORE -> Bit 0, DOLE -> Bit 1
  // Žalúzia 2: HORE -> Bit 2, DOLE -> Bit 3
  // Žalúzia 3: HORE -> Bit 4, DOLE -> Bit 5
  for (int i = 0; i < 3; i++) {
    if (lastDirection[i] == 1) pcf1 |= (0x01 << (i * 2));
    else if (lastDirection[i] == 2) pcf1 |= (0x02 << (i * 2));
  }
  
  // VETVA 2 (Zaluzie 4, 5, 6): standardne mapovanie:
  // Žalúzia 4: HORE -> Bit 0, DOLE -> Bit 1
  // Žalúzia 5: HORE -> Bit 2, DOLE -> Bit 3
  // Žalúzia 6: HORE -> Bit 4, DOLE -> Bit 5
  for (int i = 3; i < 6; i++) {
    int channel = i - 3;
  ...
  // JEDINÉ SPOLOČNÉ RELÉ POMALEJ RÝCHLOSTI (DIÓDOVÉ RELE) PRE VŠETKY ŽALÚZIE (VETVA 2, PCF2 BIT 6):
  // Aktivuje sa vzdy ak je aktivne naklapanie (bud cez microAdjusting pri SA5600, alebo pre kroky 1 a 3 v nasom casovom dvojfazovom automate!)
  bool slowSpeedActive = false;
  for (int i = 0; i < 6; i++) {
    if (lastDirection[i] != 0) {
      if (isMicroAdjusting[i] || seqLightState[i] == 2 || seqTempState[i] == 2 || timeTiltState[i] == 3) { // ZMENA: diodove rele sa pri casovom naklapani zopne EXKLUZIVNE IBA v poslednom kroku (timeTiltState == 3) kedy sa realne natacaju lamely!
  ...
  if (pcf1 != last_pcf1 || forceWrite) {
    Wire.beginTransmission(PCA_ADDR_1); 
    Wire.write(0x00); // ISOLATE SENSOR LINES TO PREVENT EMI AND CAPACITIVE GLITCHES DURING HIGH CURRENT COIL SWITCHING
  ...
  if (pcf2 != last_pcf2 || forceWrite) {
    Wire1.beginTransmission(PCA_ADDR_2); 
    Wire1.write(0x00); // ISOLATE SENSOR LINES TO PREVENT EMI AND CAPACITIVE GLITCHES DURING HIGH CURRENT COIL SWITCHING
...
void saveAllConfigurationToNVS() {
  preferences.begin("terasa", false);
  ...
  for (int i = 0; i < 6; i++) {
    char key[30];
    sprintf(key, "tu%d", i); preferences.putInt(key, tUp[i]);
    sprintf(key, "td%d", i); preferences.putInt(key, tDown[i]);
    sprintf(key, "zOff%d", i); preferences.putInt(key, zeroOffset[i]); 
    sprintf(key, "ate%d", i); preferences.putBool(key, autoTempEnable[i]);
    sprintf(key, "tmd%d", i); preferences.putInt(key, tempMode[i]);
    sprintf(key, "tlu%d", i); preferences.putFloat(key, tempLimitUp[i]);
    sprintf(key, "tld%d", i); preferences.putFloat(key, tempLimitDown[i]);
    sprintf(key, "thyst%d", i); preferences.putFloat(key, tempHysteresis[i]);
    sprintf(key, "tta%d", i); preferences.putInt(key, tTempUp[i]);
    sprintf(key, "ttd%d", i); preferences.putInt(key, tTempDown[i]);

    sprintf(key, "ale%d", i); preferences.putBool(key, autoLightEnable[i]);
    sprintf(key, "lmd%d", i); preferences.putInt(key, lightMode[i]);
    sprintf(key, "llu%d", i); preferences.putInt(key, lightLimitUp[i]);
    sprintf(key, "lld%d", i); preferences.putInt(key, lightLimitDown[i]);
    sprintf(key, "lhyst%d", i); preferences.putInt(key, lightHysteresis[i]);
    sprintf(key, "lta%d", i); preferences.putInt(key, tLightUp[i]);
    sprintf(key, "ltd%d", i); preferences.putInt(key, tLightDown[i]);

    sprintf(key, "sqle%d", i); preferences.putBool(key, seqLightEnable[i]);
    sprintf(key, "sqlp%d", i); preferences.putFloat(key, seqLightPauseTime[i]);
    sprintf(key, "sqld%d", i); preferences.putInt(key, seqLightSecondActionDir[i]);
    sprintf(key, "sqlt%d", i); preferences.putFloat(key, seqLightSecondActionTime[i]);
    sprintf(key, "sqte%d", i); preferences.putBool(key, seqTempEnable[i]);
    sprintf(key, "sqtp%d", i); preferences.putFloat(key, seqTempPauseTime[i]);
    sprintf(key, "sqtd%d", i); preferences.putInt(key, seqTempSecondActionDir[i]);
    sprintf(key, "sqtt%d", i); preferences.putFloat(key, seqTempSecondActionTime[i]);

    // Ukladanie parametrov pre dvojfazove casove naklapanie (8 predvolieb x 6 zaluzii):
    sprintf(key, "utt%d", i); preferences.putBool(key, useTimeTilt[i]);
    for (int p = 0; p < 8; p++) {
      sprintf(key, "ttd1_%d_%d", i, p); preferences.putInt(key, timeTiltDir1[i][p]);
      sprintf(key, "ttt1_%d_%d", i, p); preferences.putFloat(key, timeTiltDur1[i][p]);
      sprintf(key, "ttp_%d_%d", i, p); preferences.putFloat(key, timeTiltPause[i][p]);
      sprintf(key, "ttd2_%d_%d", i, p); preferences.putInt(key, timeTiltDir2[i][p]);
      sprintf(key, "ttt2_%d_%d", i, p); preferences.putFloat(key, timeTiltDur2[i][p]);
    }

    for (int p = 0; p < 8; p++) {
      sprintf(key, "p_%d_%d", i, p);
      preferences.putInt(key, (int32_t)tiltPresets[i][p]);
    }
  }
  preferences.end();
  Serial.println("[NVS] Vsetky parametre ulozene do Preferences.");
}

void loadNTCParams() {
  preferences.begin("terasa", true);
  ntcIntR0 = preferences.getInt("nir0", 10000);
  ntcIntBeta = preferences.getInt("nib", 3950);
  ...
  ntcIntOffset = preferences.getFloat("nioff", 0.0);
  ntcExtR0 = preferences.getInt("ner0", 10000);
  ntcExtBeta = preferences.getInt("neb", 3950);
  ...
  ntcExtOffset = preferences.getFloat("neoff", 0.0);
...
void loadConfiguration() {
  ...
  allTimeUp = preferences.getInt("atu", 30);
  allTimeDown = preferences.getInt("atd", 30);
  extTimeUpCH1 = preferences.getInt("etu1", 25);
  extTimeDownCH1 = preferences.getInt("etd1", 25);
  windLimit = preferences.getFloat("wlim", 35.0);
  windPulsesFor25kmh = preferences.getFloat("wp25", 46.0);
  pulsesPerTurn = preferences.getInt("ppturn", 2);
  windSafetyDuration = preferences.getULong("wsafedur", 60000);
  windRunTime = preferences.getInt("wruntime", 35);
  windMaxHits = preferences.getInt("wmaxhits", 3);
  tempIntMin = preferences.getFloat("timin", 99.9);
  tempIntMax = preferences.getFloat("timax", -99.9);
  tempExtMin = preferences.getFloat("temin", 99.9);
  tempExtMax = preferences.getFloat("temax", -99.9);
  microAdjustThreshold = preferences.getInt("mcth", 80);

  for (int i = 0; i < 6; i++) {
    char key[30];
    sprintf(key, "tu%d", i); tUp[i] = preferences.getInt(key, 20);
    sprintf(key, "td%d", i); tDown[i] = preferences.getInt(key, 20);
    sprintf(key, "zOff%d", i); zeroOffset[i] = preferences.getInt(key, 0);
    autoTempEnable[i] = preferences.getBool((String("ate")+i).c_str(), true);
    sprintf(key, "tmd%d", i); tempMode[i] = preferences.getInt(key, 0);
    sprintf(key, "tlu%d", i); tempLimitUp[i] = preferences.getFloat(key, 22.0);
    sprintf(key, "tld%d", i); tempLimitDown[i] = preferences.getFloat(key, 26.0);
    sprintf(key, "thyst%d", i); tempHysteresis[i] = preferences.getFloat(key, 0.5);
    sprintf(key, "tta%d", i); tTempUp[i] = preferences.getInt(key, 15);
    sprintf(key, "ttd%d", i); tTempDown[i] = preferences.getInt(key, 15);

    autoLightEnable[i] = preferences.getBool((String("ale")+i).c_str(), true);
    sprintf(key, "lmd%d", i); lightMode[i] = preferences.getInt(key, 0);
    sprintf(key, "llu%d", i); lightLimitUp[i] = preferences.getInt(key, 250);
    sprintf(key, "lld%d", i); lightLimitDown[i] = preferences.getInt(key, 750);
    sprintf(key, "lhyst%d", i); lightHysteresis[i] = preferences.getInt(key, 50);
    sprintf(key, "lta%d", i); tLightUp[i] = preferences.getInt(key, 15);
    sprintf(key, "ltd%d", i); tLightDown[i] = preferences.getInt(key, 15);

    sprintf(key, "sqle%d", i); seqLightEnable[i] = preferences.getBool(key, false);
    sprintf(key, "sqlp%d", i); seqLightPauseTime[i] = preferences.getFloat(key, 10.0);
    sprintf(key, "sqld%d", i); seqLightSecondActionDir[i] = preferences.getInt(key, 1);
    sprintf(key, "sqlt%d", i); seqLightSecondActionTime[i] = preferences.getFloat(key, 5.0);
    
    // OPRAVA BUGU: Korektne priradenie premennej pre pauzu dvojfázovej teplotnej automatiky
    sprintf(key, "sqte%d", i); seqTempEnable[i] = preferences.getBool(key, false);
    sprintf(key, "sqtp%d", i); seqTempPauseTime[i] = preferences.getFloat(key, 10.0);
    sprintf(key, "sqtd%d", i); seqTempSecondActionDir[i] = preferences.getInt(key, 1);
    sprintf(key, "sqtt%d", i); seqTempSecondActionTime[i] = preferences.getFloat(key, 5.0);

    // Nacitavanie z NVS pre dvojfazove casove naklapanie (8 predvolieb x 6 zaluzii):
    sprintf(key, "utt%d", i); useTimeTilt[i] = preferences.getBool(key, false);
    for (int p = 0; p < 8; p++) {
      sprintf(key, "ttd1_%d_%d", i, p); timeTiltDir1[i][p] = preferences.getInt(key, 2); // default: 2 (DOLE)
      sprintf(key, "ttt1_%d_%d", i, p); timeTiltDur1[i][p] = preferences.getFloat(key, 1.50); // default: 1.50s
      sprintf(key, "ttp_%d_%d", i, p); timeTiltPause[i][p] = preferences.getFloat(key, 0.50); // default: 0.50s
      sprintf(key, "ttd2_%d_%d", i, p); timeTiltDir2[i][p] = preferences.getInt(key, 1); // default: 1 (HORE)
      sprintf(key, "ttt2_%d_%d", i, p); timeTiltDur2[i][p] = preferences.getFloat(key, 0.10 * (p + 1)); // default: linearny odstup presetov pre lahsie prve nastavenie
    }

    for (int p = 0; p < 8; p++) {
      sprintf(key, "p_%d_%d", i, p);
      tiltPresets[i][p] = (int32_t)preferences.getInt(key, 0);
    }
  }
  preferences.end();
}

// --- POMOCNA FUNKCIA NA OCHRANU JSON PRED NAN/INF HODNOTAMI ---
...
String makeStatusJSON() {
  String json;
  json.reserve(6144); // Zvysena rezervacia pre obsiahlejsiu tabulku 2D predvolieb prenosu
  json = "{";
  ...
  json += "],\"tu\":["; for(int i=0; i<6; i++) { json += String(tUp[i]) + (i<5?",":""); }
  json += "],\"td\":["; for(int i=0; i<6; i++) { json += String(tDown[i]) + (i<5?",":""); }
  json += "],\"zOff\":["; for(int i=0; i<6; i++) { json += String(zeroOffset[i]) + (i<5?",":""); }
  json += "],\"ate\":["; for(int i=0; i<6; i++) { json += String(autoTempEnable[i] ? "1" : "0") + (i<5?",":""); }
  json += "],\"tmd\":["; for(int i=0; i<6; i++) { json += String(tempMode[i]) + (i<5?",":""); }
  json += "],\"tlu\":["; for(int i=0; i<6; i++) { json += String(tempLimitUp[i], 1) + (i<5?",":""); }
  json += "],\"tta\":["; for(int i=0; i<6; i++) { json += String(tTempUp[i]) + (i<5?",":""); }
  json += "],\"tld\":["; for(int i=0; i<6; i++) { json += String(tempLimitDown[i], 1) + (i<5?",":""); }
  json += "],\"ttd\":["; for(int i=0; i<6; i++) { json += String(tTempDown[i]) + (i<5?",":""); } 
  json += "],\"thyst\":["; for(int i=0; i<6; i++) { json += String(tempHysteresis[i], 1) + (i<5?",":""); }

  json += "],\"ale\":["; for(int i=0; i<6; i++) { json += String(autoLightEnable[i] ? "1" : "0") + (i<5?",":""); }
  json += "],\"lmd\":["; for (int i=0; i<6; i++) { json += String(lightMode[i]) + (i<5?",":""); }
  json += "],\"llu\":["; for (int i=0; i<6; i++) { json += String(lightLimitUp[i]) + (i<5?",":""); }
  json += "],\"lta\":["; for (int i=0; i<6; i++) { json += String(tLightUp[i]) + (i<5?",":""); }
  json += "],\"lld\":["; for (int i=0; i<6; i++) { json += String(lightLimitDown[i]) + (i<5?",":""); }
  json += "],\"ltd\":["; for (int i=0; i<6; i++) { json += String(tLightDown[i]) + (i < 5 ? "," : ""); } 
  ...
  // Pridanie nasich novych 2D poli pre casovu automatiku naklapania polohy:
  json += "],\"utt\":["; for(int i=0; i<6; i++) { json += String(useTimeTilt[i] ? "1" : "0") + (i<5?",":""); }
  ...
  json += "\"labels\":["; for (int i = 0; i < graphCount; i++) { json += "\"" + historyTime[i] + "\"" + (i < graphCount - 1 ? "," : ""); }
...
void setup() {
    ...
    if (request->hasParam("i") && request->hasParam("t")) {
      ...
      if (i >= 0 && i < 6) {
        ...
        if (type == 1 && seqLightEnable[i]) {
          ...
          runUntil[i] = millis() + dur; runDirection[i] = 2; lightActionTriggered[i] = true;
  ...
  server.on("/setpreset", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if (i >= 0 && i < 6 && p >= 0 && p < 8) {
      if (useTimeTilt[i]) {
        ...
        // Klasicke naklapanie podla snimaca polohy SA5600
  ...
  server.on("/saverow", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (i >= 0 && i < 6) {
      ...
      for (int p = 0; p < 8; p++) { 
        char pParam[10]; sprintf(pParam, "p%d", p); 
...
void loop() {
  ...
  if (!safetyActive) {
    if (extBtnAutoActive) {
      if (rawUp == LOW || rawDown == LOW) {
        ...
        seqLightState[0] = 0;
        seqTempState[0] = 0;
        timeTiltState[0] = 0; // RESET TIMED TILT
      }
    } else {
  ...
  // --- NEBLOKUJUCI AUTOMATICKY WIFI WATCHDOG PROTI VYPADKOM A REBOOTOM ROUTRA ---
  ...
  if (now - lastSensorMillis >= 1000) {
    unsigned long durationMs = now - lastSensorMillis; lastSensorMillis = now;
    noInterrupts(); unsigned long pulses = pulseCount; pulseCount = 0; interrupts();
    float currentWindSpeed = 0.0;
    if (pulses > 0 && durationMs > 0) { float rps = (float)pulses / ((float)pulsesPerTurn * (durationMs / 1000.0)); currentWindSpeed = rps * (25.0 / (windPulsesFor25kmh / (float)pulsesPerTurn)); }
    windSpeed = windSpeed + 0.5 * (currentWindSpeed - windSpeed); if (windSpeed > windSpeedMax) windSpeedMax = windSpeed;
    float maxFound24h = 0.0; for (int i = 0; i < graphCount; i++) { if (historyWind[i] > maxFound24h) maxFound24h = historyWind[i]; }
    ...
    if (windSpeed >= windLimit) {
      ...
      if (windHitCounter >= windMaxHits && !safetyActive) { 
        safetyActive = true; windSafetyUntil = now + windSafetyDuration; windRunUntil = now + ((unsigned long)windRunTime * 1000); sendWindAlarmEmail(windSpeed, windLimit); 
  ...
  if (timeValid) {
    ...
    // OCHRANNÁ SPÄTNÁ SYNCHRONIZÁCIA ČASOVÝCH ZNAČIEK:
    // Ak sa po zapnutí inicializovali grafy s "--:--", spätne ich zosynchronizujeme s reálnym časom.
  ...
  // --- AUTOMATICKA INICIALIZACIA GRAFOV PO 30 SEKUNDACH OD BOOTU ---
  // Grafy sa teraz inicializujú VŽDY po 30 sekundách (aj bez NTP), aby weby neostali prázdne.
  if (!graphInitialized && (now - bootMillis > 30000)) {
  ...
  for (int i = 0; i < 6; i++) {
    if (isTargetingPos[i] && !safetyActive) {
      int32_t posDiff = targetPosition[i] - getEffectivePosition(i); int32_t absDiff = abs(posDiff);
  ...
  for (int i = 0; i < 6; i++) {
    if (timeTiltState[i] > 0 && !safetyActive) {
      int p = timeTiltPresetIndex[i]; // Zistenie priradeneho presetu rolety i
      if (timeTiltState[i] == 1) { // Krok 1 (Prvy pohyb nastavenym smerom)
        if (millis() >= timeTiltTimer[i]) {
          // Krok 1 dokonceny, prejdi na Pauzu (Krok 2)
          timeTiltState[i] = 2;
          timeTiltTimer[i] = millis() + (unsigned long)(timeTiltPause[i][p] * 1000.0);
          runDirection[i] = 0;
          runUntil[i] = 0;
      ...
      else if (timeTiltState[i] == 2) { // Krok 2 (Pauza)
        if (millis() >= timeTiltTimer[i]) {
          // Pauza skoncila, prejdi na Krok 3 (Druhy pohyb) - vyuziva sa individualne nastaveny čas 3 z predvolby p
          ...
          if (dur2 > 0.01) {
            ...
            // Druhy krok ma nulovy alebo prilis kratky cas, ukonci
            timeTiltState[i] = 0;
            runDirection[i] = 0;
            runUntil[i] = 0;
          }
        } else {
          // Sme v pauze, drz motory vypnute
          runDirection[i] = 0;
          runUntil[i] = 0;
        }
      } 
      else if (timeTiltState[i] == 3) { // Krok 3 (Druhy pohyb - natacanie na pomalej rychlosti s diodou!)
        if (millis() >= timeTiltTimer[i]) {
          // Krok 3 ukonceny, koniec celej sekvencie naklopenia
          timeTiltState[i] = 0;
          runDirection[i] = 0;
          runUntil[i] = 0;
        } else {
          // Pokracuj v pohybe vybranym smerom
          runDirection[i] = timeTiltDir2[i][p];
          runUntil[i] = timeTiltTimer[i];
        }
      }
  ...
  if (globalAutoEnable && withinTimeWindow && !safetyActive && !extBtnAutoActive) {
    for (int i = 0; i < 6; i++) {
      if (!isTargetingPos[i] && timeTiltState[i] == 0) {
        ...
        // Riadenie dvoch faz pre Svetelnu Automatiku
        if (seqLightEnable[i]) {
          if (seqLightState[i] == 1 && now >= seqLightTimer[i]) {
            if (!safetyActive) {
              ...
              runUntil[i] = now + dur2;
              seqLightState[i] = 2; // Prechod do stavu 2 (beh druhej fazy s diodou / pomalou rychlostou)
            ...
            seqLightState[i] = 0; // Druha faza ukoncena
          ...
          if (runDirection[i] == 0 && seqLightState[i] == 0 && seqTempState[i] == 0) {
            ...
            if (tCurrent > -50.0 && tCurrent < 100.0) {
              if (tCurrent >= (tempLimitDown[i] + tempHysteresis[i]) && !tempActionTriggered[i]) { 
                ...
                unsigned long dur = (unsigned long)tTempUp[i] * 1000; 
                runUntil[i] = now + dur; 
                runDirection[i] = 1; 
        ...
        // Riadenie dvoch faz pre Teplotnu Automatiku
        if (seqTempEnable[i]) {
          if (seqTempState[i] == 1 && now >= seqTempTimer[i]) {
            if (!safetyActive) {
              ...
              runUntil[i] = now + dur2;
              seqTempState[i] = 2; // Prechod do stavu 2 (beh druhej fazy)
            ...
            seqTempState[i] = 0; // Druha faza ukoncena
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (i == 0 && !extBtnAutoActive && (rawUp == LOW || rawDown == LOW)) {
      ...
      if (allHore) moveUp = true; else if (allDole) moveDown = true;
    ...
    if (motorTriggerTime[i] > 0) {
      if (now >= motorTriggerTime[i]) {
        motorTriggerTime[i] = 0; 
      } else {
        desiredDirection = 0; 
      }
    }

    if (desiredDirection != lastDirection[i] && lastDirection[i] != 0 && desiredDirection != 0) { safeSwitchMillis[i] = now; lastDirection[i] = 0; runUntil[i] = 0; runDirection[i] = 0; isTargetingPos[i] = false; timeTiltState[i] = 0; desiredDirection = 0; }
    if (desiredDirection != 0 && lastDirection[i] == 0) { if (now - safeSwitchMillis[i] < DIRECTION_DELAY) desiredDirection = 0; }
    if (desiredDirection == 0) { if (lastDirection[i] != 0) { safeSwitchMillis[i] = now; lastDirection[i] = 0; } } else lastDirection[i] = desiredDirection;
  }
